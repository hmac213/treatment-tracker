...
export async function GET(req: NextRequest) {
  ...
  if (videosError) {
    console.error('Failed to fetch bonus content videos:', videosError);
    return NextResponse.json({ error: 'Failed to fetch bonus content videos' }, { status: 500 });
  }

  // Get bonus content positions
  const { data: positions, error: positionsError } = await supabase
    .from('bonus_content_positions')
    .select('*');

  if (positionsError) {
    console.error('Failed to fetch bonus content positions:', positionsError);
    return NextResponse.json({ error: 'Failed to fetch bonus content positions' }, { status: 500 });
  }
...
export async function POST(req: NextRequest) {
  ...
  if (position) {
    ...
    if (posError) {
      console.error('Failed to save bonus content position:', posError);
