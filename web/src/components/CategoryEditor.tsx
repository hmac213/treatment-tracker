...
interface CategoryEditorProps {
  categoryVideos: Record<string, CategoryVideo[]>;
  onUpdate: () => void;
}
...
export function CategoryEditor({ categoryVideos, onUpdate }: CategoryEditorProps) {
  const [editingCategory, setEditingCategory] = useState<string | null>(null);
  const [categoryForms, setCategoryForms] = useState<Record<string, CategoryVideo[]>>({});
  ...
  const startEditing = (category: string) => {
    ...
    setCategoryForms({
      ...categoryForms,
      [category]: categoryVideos[category] ? [...categoryVideos[category]] : [],
    });
  };
  ...
  const updateVideo = (category: string, index: number, field: keyof CategoryVideo, value: string | number) => {
    const videos = categoryForms[category] || [];
  ...
  const saveCategory = async (category: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Failed to save category videos');
      }
    } catch (error) {
      console.error('Error saving category videos:', error);
      alert('Error saving category videos');
    } finally {
      setSaving(null);
    }
  };
  ...
  return (
    <div className="space-y-6">
      {CATEGORIES.map(({ key, label }) => {
        ...
        const videos = isEditing ? (categoryForms[key] || []) : (categoryVideos[key] || []);
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Videos that appear when clicking on the {label} category label
                  </CardDescription>
                </div>
                {!isEditing && (
            ...
            <CardContent>
              {isEditing ? (
                    ...
                    <div key={index} className="p-4 bg-gray-50 rounded-lg border border-gray-200 space-y-3">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          placeholder="e.g., Introduction to Skincare"
                ...
                <div className="space-y-2">
                  {videos.length === 0 ? (
                    <p className="text-sm text-gray-500 italic">No videos configured for this category.</p>
                  ) : (
                    videos
