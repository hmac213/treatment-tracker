-- Migration: Update data structure to match the actual treatment flowchart (SAFE VERSION)
-- This creates the true DAG structure with shared nodes and proper conditional paths
-- Handles existing data safely with ON CONFLICT clauses
...
-- Use a more robust approach with individual inserts
DO $$
DECLARE
    node_record RECORD;
    category_assignments TEXT[][] := ARRAY[
        -- Single category assignments
        ['calendula', 'skincare'],
        ['silvadene', 'skincare'],
        ['mepilex', 'skincare'],
        ['eat_any', 'nutrition'],
        ...
        ['opioid', 'pain'],
        ['gabapentin', 'pain'],
        -- SHARED NODES (multiple category assignments)
    ...
    assignment TEXT[];
BEGIN
    FOREACH assignment SLICE 1 IN ARRAY category_assignments
    LOOP
        INSERT INTO public.node_categories (node_id, category)
        SELECT n.id, assignment[2]
        FROM public.nodes n
        WHERE n.key = assignment[1]
        ON CONFLICT (node_id, category) DO NOTHING;
    END LOOP;
END $$;

-- Create the DAG structure with proper conditional edges
-- Use a safer approach that handles existing edges
DO $$
DECLARE
    root_id uuid;
    calendula_id uuid;
    ...
    gabapentin_id uuid;
BEGIN
    -- Get node IDs (with error checking)
    ...
    IF root_id IS NULL OR calendula_id IS NULL OR eat_any_id IS NULL OR baking_2x_id IS NULL OR lidocaine_id IS NULL THEN
        RAISE EXCEPTION 'Some required nodes are missing. Check that all nodes exist before running this migration.';
    END IF;
    ...
    IF silvadene_id IS NOT NULL AND mepilex_id IS NOT NULL THEN
        INSERT INTO public.edges (parent_id, child_id, unlock_type, unlock_value, description, weight) VALUES
    ...
    IF liquid_diet_id IS NOT NULL AND tube_feeding_id IS NOT NULL THEN
        INSERT INTO public.edges (parent_id, child_id, unlock_type, unlock_value, description, weight) VALUES
    ...
    IF supportive_id IS NOT NULL AND baking_4x_id IS NOT NULL AND mugard_spot_id IS NOT NULL THEN
        INSERT INTO public.edges (parent_id, child_id, unlock_type, unlock_value, description, weight) VALUES
    ...
    IF dox_morph_id IS NOT NULL AND opioid_id IS NOT NULL AND gabapentin_id IS NOT NULL THEN
        INSERT INTO public.edges (parent_id, child_id, unlock_type, unlock_value, description, weight) VALUES
    ...
    -- Only create these if all required nodes exist
    IF baking_4x_id IS NOT NULL AND dox_morph_id IS NOT NULL AND supportive_id IS NOT NULL AND opioid_id IS NOT NULL AND mugard_spot_id IS NOT NULL THEN
...
-- Create symptoms that match our unlock conditions
